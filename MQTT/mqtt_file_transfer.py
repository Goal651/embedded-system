...
def on_connect(client, userdata, flags, rc):
    ...
    client.subscribe(
        "/instructor_gabriel"
    )  # Listen for messages from Instructor Gabriel
...
# Display text messages from the instructor
client = mqtt.Client()
...
client.loop_start()
student_topic = "/wilson_goal"
while True:
    option = (
        input("Type 'text' to send a message or 'image' to send a picture: ")
        .strip()
        .lower()
    )
    if option == "text":
